  ...
  - sid: 0x62
    ...
    dids_observed:
      ...
      - did: 0xF301
        ...
        confidence: high
        notes: "Dynamic DID slot, NOT a static signal. Techstream creates short-lived compositions in this slot via service 0x2C 01 (defineByIdentifier), polls the slot via 0x22, then clears via 0x2C 03 when the Data List selection changes. Current contents change with every Data List edit; do not record observed values here."
      - did: 0xF302
        name: DynamicSlot2
...
**Negative responses (`0x7F`)** were not observed in this session. They may show up when probing extended-session-only DIDs from the default session, or when probing source DIDs that don't exist.
